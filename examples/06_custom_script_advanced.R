...
writeLines(c(
  ...
  '  } else {',
  '    # Quadratische Terme fuer alle numerischen Praediktoren',
  '    preds <- setdiff(names(train), "mpg")',
  '    quad <- paste0("I(", preds, "^2)")',
  '    f <- as.formula(paste("mpg ~", paste(c(preds, quad), collapse = " + ")))',
  '    mod <- lm(f, data = train)',
  '  }',
  '',
